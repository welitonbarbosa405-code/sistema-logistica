...
PASTA_XML = os.path.join('BRITO TRANSPORTES', 'CTE')
PASTA_FATURAS = os.path.join('BRITO TRANSPORTES', 'FATURAS')
...
def encontrar_pdf_cte(numero_cte):
    ...
    # Remove zeros à esquerda para comparação mais flexível
    numero_cte_limpo = str(int(numero_cte)) if numero_cte.isdigit() else numero_cte
    
    for arquivo in os.listdir(PASTA_XML):
        if arquivo.lower().endswith('.pdf'):
            # Procura pelo número do CTE no nome do arquivo (com diferentes variações)
            if numero_cte in arquivo or numero_cte_limpo in arquivo:
...
def encontrar_pdf_fatura(numero_cte):
    ...
    for arquivo in os.listdir(PASTA_FATURAS):
        if arquivo.lower().endswith('.pdf'):
            # Procura pelo número do CTE no nome do arquivo da fatura (com diferentes variações)
            if numero_cte in arquivo or numero_cte_limpo in arquivo:
